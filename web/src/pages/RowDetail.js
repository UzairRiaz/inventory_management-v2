...
export default function RowDetail() {
  ...
  const { token } = useAuth();
  const { title = 'Details', details = {}, deleteAction } = location.state || {};
  ...
  const handleDelete = async () => {
    ...
    const confirmed = window.confirm('This action cannot be undone. Delete this record?');
    if (!confirmed) return;
  ...
  return (
    <Screen>
      <Section title={title} icon="DET">
        {detailRows.length === 0 ? (
          <div className="meta-text">No details available</div>
        ) : (
          <RecordList
            title="Row Details"
            data={detailRows}
